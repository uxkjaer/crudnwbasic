/*global location */
sap.ui.define([
	"agilux/zsimpletest/controller/BaseController",
	"sap/ui/model/json/JSONModel",
	"agilux/zsimpletest/model/formatter",
	"sap/m/MessageBox",
	"sap/m/MessageToast"
], function (BaseController, JSONModel, formatter, MessageBox, MessageToast) {
	"use strict";

	return BaseController.extend("agilux.zsimpletest.controller.Detail", {
		...
		onDelete: function () {
			...
			var oViewModel = this.getModel("detailView"),
				...
				sObjectHeader = this._oODataModel.getProperty(sPath + "/Text"),
		...
		_onObjectMatched: function (oEvent) {
			...
			this.getModel().metadataLoaded().then(function () {
				var sObjectPath = this.getModel().createKey("headerSet", oParameter);
		...
		onCreate: function () {
			var oItem = this.getView().byId("lineItemsList").getBindingInfo('items').template.clone();
			var sId = this.getView().byId("lineItemsList").getItems().length + 1
			var oContextPath = this.getModel().createEntry("/itemSet", {
				properties: {
					ParentId: this.getView().getBindingContext().getProperty("Id")
					, Id: sId.toString()
				}, groupId: 'test', 
				headers: {
					"content-ID": this.getView().getBindingContext().getProperty("Id")
					
				}
			})

			oItem.setBindingContext(oContextPath);

			this.getView().byId("lineItemsList").addItem(oItem)

		},
		onSave: function () {
			this.getModel().submitChanges({ groupId: 'test' });
		},
		...
		onDeleteItem: function (oEvent) {
			...
			this.getView().byId("lineItemsList").removeItem(oEvent.getParameter("listItem"))
		}
	});
});